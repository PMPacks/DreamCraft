...
class PlotLevelSettings
{
    ...
    public function __construct($name, $settings = []) {
        $this->name = $name;
        if (!empty($settings)) {
            $this->roadBlock = self::parseBlock($settings, "RoadBlock", new Block(10));
            $this->wallBlock = self::parseBlock($settings, "WallBlock", new Block(44));
            $this->plotFloorBlock = self::parseBlock($settings, "PlotFloorBlock", new Block(10));
            $this->plotFillBlock = self::parseBlock($settings, "PlotFillBlock", new Block(10));
