        ...
        foreach ($enchants as $enchant) {
            ...
            if ($result === true || $check !== true) {
                ...
                if (!$found) {
                    ...
                    $item->setNamedTag($tag);
                    $item->setCustomName(TextFormat::RESET."§l§e".$item->getName() . "\n" . $this->getRarityColor($enchant->getRarity()) . $enchant->getName() . " " . $level);
					$item->setLore(array("§2⬛§a⬛§3⬛§b⬛§4⬛§c⬛§5⬛§d⬛§6⬛§7⬛§f♦§7⬛§6⬛§d⬛§5⬛§c⬛§4⬛§b⬛§3⬛§a⬛§2⬛", "§l§cVẬT PHẨM CÓ GIÁ TRỊ CAO KHÔNG ĐƯỢC LÀM MẤT", "§l§6VẬT PHẨM KHÔNG ĐƯỢC BÁN TẠI SHOP CHỈ CÓ THỂ NHẬN ĐƯỢC KHI MUA BẰNG CARD", "§2⬛§a⬛§3⬛§b⬛§4⬛§c⬛§5⬛§d⬛§6⬛§7⬛§f♦§7⬛§6⬛§d⬛§5⬛§c⬛§4⬛§b⬛§3⬛§a⬛§2⬛"));
                }
                if ($sender !== null) {
            ...
            if ($sender !== null) {
                if ($result == self::NOT_COMPATIBLE) {
                    $sender->sendMessage(TextFormat::RED . "Phù Phép Này Không Phù Hợp Với Vật Phẩm Bạn Đang Cầm.");
                }
                if ($result == self::NOT_COMPATIBLE_WITH_OTHER_ENCHANT) {
                    $sender->sendMessage(TextFormat::RED . "Vật Phầm Này Đã Có 1 Phù Phép Không Phù Hợp Với Phù Phép Này.");
                }
                if ($result == self::MAX_LEVEL) {
                    $sender->sendMessage(TextFormat::RED . "Cấp Độ Cao Nhất Của Phù Phép Này Là " . $this->getEnchantMaxLevel($enchant) . ".");
                }
            }
            continue;
        }
