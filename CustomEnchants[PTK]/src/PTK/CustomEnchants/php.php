...
use PTK\CustomEnchants\Tasks\GoeyTask;
use PTK\CustomEnchants\Tasks\GrapplingTask;
use PTK\CustomEnchants\Tasks\HallucinationTask;
use PTK\CustomEnchants\Tasks\MoltenTask;
            ...
            if ($event instanceof PlayerToggleSneakEvent) {
                ...
                if ($enchantment !== null) {
                    if (isset($this->plugin->flying[strtolower($entity->getName())]) && $this->plugin->flying[strtolower($entity->getName())] > time()) {
						...
						$t = str_repeat(" ", 85);
                            $entity->sendPopup($t.TextFormat::RED . "JatPack Đã Tắt.");
                        } else {
						$t = str_repeat(" ", 85);
                            $entity->sendPopup($t.TextFormat::RED . "Bạn Không Thể Tắt JetPack Khi Đang Bay.");
                        }
                    } else {
						...
						$t = str_repeat(" ", 85);
                            if (!isset($this->plugin->jetpackcd[strtolower($entity->getName())]) || $this->plugin->jetpackcd[strtolower($entity->getName())] <= time()) {
                                ...
                                $entity->sendPopup($t.TextFormat::GREEN . "JetPack Đã Kích Hoạt. Ấn Ngồi Để Tắt.");
						...
						$t = str_repeat(" ", 85);
                            $entity->sendPopup($t.TextFormat::RED . "JetPack Không Thể Sử Dụng Trong Thế Giới Này.");
                        }
                    }
                }
            }
